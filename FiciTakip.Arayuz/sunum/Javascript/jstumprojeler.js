...
function fn_GuncelleModalKapat() {
...
function AramaProjeDurumListe() {

    var ddlSehir = $("select#listeProjeDurum");

    $.ajax({
        type: "POST",
        url: "api/RaporDurumProje",
        data: "{}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        beforeSend: function () {
        ...
        success: function (msg) {
            ...
            if (msg.Sonuc == "1") {
                ...
                for (var iSayac = 0; iSayac < vProjeDizisi.length; iSayac++) {
                    ddlSehir.append($("<option></option>").val(vProjeDizisi[iSayac].TabloId).html(vProjeDizisi[iSayac].DurumAdi));
                }
            }
        },
        complete: function () {
            AramaProjeTuruListele();
        }
    });
}

function AramaProjeTuruListele() {

    var ddlSehir = $("select#listeAramaProjeTuru");
...
function AramaSehirListele() {
       
    var ddlListe = $("select#listeAramaSehir");
    ...
    $.ajax({
        ...
        success: function (msg) {
            ...
            if (msg.Sonuc == "1") {
                ...
                ddlListe.append($("<option></option>").val('-1').html('HEPSİ'));
...
function jsProjeAra() {
    var v_ProjeTuru = $("select#listeAramaProjeTuru").val();
    var v_Sehir = $("select#listeAramaSehir").val();
    var v_KayitTuru = $("select#listeKayitTuru").val();
    var v_ProjeDurum = $("select#listeProjeDurum").val();
        ...
        success: function (msg) {
            ...
            if (msg.Sonuc == "1") {
                ...
                for (var iSayac = 0; iSayac < vYanitDizi.length; iSayac++) {
                    ...
                    var v_Uzunluk = vYanitDizi[iSayac].Uzunluk;

                    if (v_Uzunluk.length > 0)
                    {
                        v_Uzunluk = v_Uzunluk / 1000;
                    }
                   ...
                   // content += "<td style='text-align:right'>" + vSayac + "</td>";
                    ...
                    content += "<td>" + vYanitDizi[iSayac].IlceAdi + "</td>";
                    content += "<td style='display: none!important;'>" + vYanitDizi[iSayac].ProjeDurumAdi + "</td>";
                    content += "<td>" + vYanitDizi[iSayac].ProjeAdi + "</td>";
                    content += "<td style=\"text-align: right;\">" + v_Uzunluk + "</td>";
                    content += "<td style='text-align:center'>" + vYanitDizi[iSayac].DosyaAdi + "</td>";
            ...
            settings.columnDefs= [
                    {
                        ...
                        <span class="dropdown" style='cursor:pointer !important;'>
                            ...
                            <div class="dropdown-menu dropdown-menu-right">
                                <a class="dropdown-item" href="#" onclick="jsProjeGoruntuleGuncelle('`+ full[0] + `');"><i class="la la-edit"></i>Bilgileri Görüntüle</a>
                                <a class="dropdown-item" href="#" onclick="jsHaritadaGoruntule('`+ full[0] + `');"><i class="la la-map-marker"></i>Harita Görüntüle</a>
                                <a class="dropdown-item" href="#" onclick="jsKayitGecmisi('`+ full[0] + `');"><i class="la la-history"></i>Kayıt Bilgileri</a>
                                <a class="dropdown-item" href="#" onclick="jsProjeSilOnay('`+ full[0] + `');"><i class="la la-remove"></i>Projeyi Sil</a>
