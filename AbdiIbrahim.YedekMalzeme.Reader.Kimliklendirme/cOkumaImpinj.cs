    ...
    public class cOkumaImpinj
    {
                ...
                try
                {
                    ...
                    foreach (var item in GetCollectTagData())
                    {
                        try
                        {
                            ...
                            {
                                tbl04okuma _Temp = session.Query<tbl04okuma>().FirstOrDefault(w => w.gelenepc.Equals(item.Key));
        ...
        static void ConnectToReader()
        {
            try
            {
                ...
                using (Session session =XpoManager.Instance.GetNewSession())
                {
                    tblreaderkapiparam _readerparam = session.Query<tblreaderkapiparam>().SingleOrDefault(a => a.aktif == 1);
                    READER_HOSTNAME = _readerparam.kapiip;
                }
